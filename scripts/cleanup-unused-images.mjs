/**
 * Deletes unused images from public/assets/images.
 * Keeps only files referenced in MDX, components, or src/assets.
 */
import fs from 'fs';
...
const pubDir = path.join(root, 'public', 'assets', 'images');
if (!fs.existsSync(pubDir)) {
  console.log('No public/assets/images folder found.');
  process.exit(0);
}
...
for (const file of fs.readdirSync(pubDir)) {
  if (!refs.has(file)) {
    fs.unlinkSync(path.join(pubDir, file));
    deleted++;
  }
}

const remaining = fs.readdirSync(pubDir);
console.log(`Deleted ${deleted} unused image(s). ${remaining.length} kept in public/assets/images.`);
